...
const storageId = (groupedByProject: Map<string, User[]>, repo: Repo) => {
...
const RepoRenderer = ({
  ...
  groupedByProject: Map<string, User[]>;
  repoInfo: RepoInfoMap;
  projects: Repo[];
  firstWithOneStar: Repo;
}) => {
  const users = groupedByProject.get(repo.name) || [];
  ...
  return (
    <>
      ...
      {users.length === 1 &&
        ...
        lastSeen !== storageId(groupedByProject, repo) && (
  ...
  React.useEffect(() => {
    ...
    if (windowFocus) {
      localStorage.setItem(
        "github-activity-last-seen",
        storageId(groupedByProject, first)
      );
    }
            ...
            <>
              ...
              {fromUsersRepos.map((repo) => (
                <RepoRenderer
                  key={`repo-${repo.id}`}
                  lastSeen={lastSeen}
                  firstWithOneStar={firstWithOneStar}
